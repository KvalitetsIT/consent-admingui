...
public class ViewController {
    ...
    public String frontPage(Model model) {
    	...
    	List<String> municipalities = tos.stream().map(ConsentTemplateTO::getMunicipality).distinct().collect(Collectors.toList());
    ...
    public String updateConsentTemplate(Model model, @ModelAttribute("request") UploadFileRequest request) {
    	try {
    		...
    		req.setContent(Base64.encodeBase64String(file));
